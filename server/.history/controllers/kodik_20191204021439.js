...
const animeList = body => {
    ...
    if (getJson) {
        ...
        getJson.forEach(function(item, i, arr) {
            ...
            list += `<playlist_url>http://animerise.ddns.net:3000/list/${encoded}/${item["last_episode"]}/${item["last_season"]} </playlist_url>\n`;
            list += `<description>\n`;
...
exports.search = function(request, res) {
    let urls = new URL("http://animerise.ddns.net:3000/" + request.url);
    ...
    if (title != "null")
        ...
        url = `https://kodikapi.com/list?token=${token}&title=${title}${params}`;
...
exports.list = function(request, res) {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        list += `<stream_url>http://animerise.ddns.net:3000/get_link/${link}/${i}/${season}</stream_url>\n`;
